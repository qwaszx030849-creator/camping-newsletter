...
- 시설 시공·모듈러하우스·카라반 설치 업체가 작성한 성공 사례형 홍보글
...
카테고리: 시즌운영, 시설안전, 물놀이안전, 위생관리, 운영노하우, 매출전략, 시설개선, 산업동향, 고객관리, 후기인사이트, 리스크관리 중 택 1"""
...
- 기사 제목을 반복하지 말고 운영자가 이번 주 실행할 행동을 최소 1개 포함
...
_STRONG_POSITIVE = [
    ...
    "장마", "집중호우", "폭염", "침수", "배수로", "대피",
    "물놀이 안전", "식중독", "온열질환", "태풍", "누전",
    # 산업 동향 (뉴스)
...
def _classify_category(item: ContentItem) -> str:
    ...
    if any(w in text for w in ["수영장", "물놀이", "익수", "구명조끼"]):
        return "물놀이안전"
    if any(w in text for w in ["식중독", "냉장", "교차오염", "원산지", "식품 위생"]):
        return "위생관리"
    if any(w in text for w in ["침수", "배수로", "분전함", "누전", "대피로", "사면"]):
        return "시설안전"
